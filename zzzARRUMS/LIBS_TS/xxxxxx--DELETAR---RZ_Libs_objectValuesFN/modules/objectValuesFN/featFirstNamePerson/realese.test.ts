...
import { _FirstNamePerson } from "./realese.ts";
import {
  firstNameFake,
...
const makeSut = (d: ArgsSut) => {
  return {
    sutFirstNamePerson: _FirstNamePerson(d),
  };
};
...
Deno.test("[ Fail >> FirstNamePerson { lengthTextMin } ] deve retornar a class de Error generica ", () => {
  ...
  try {
    ...
    console.log("Ok: instancia do erro é >> Error !!!");
  }
});
...
const controllerFirstNamePersonMock = (d: ArgControllerSut) => {
  try {
    const newObj = {
      primeiroNome: _FirstNamePerson(d.primeiroNome),
      key2: d.key2,
    };
    return {
      data: newObj,
